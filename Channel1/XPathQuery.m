...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult)
{
	...
	{
        unsigned char * curr = currentNode->content;
        
        //NSLog(@"Current Content: %s", curr);
        
		NSString *currentNodeContent =
			[NSString stringWithCString:(const char *)curr encoding:NSUTF8StringEncoding];
...
NSArray *PerformHTMLXPathQuery(NSData *document, NSString *query)
{
    ...
    /* Load XML document */
	doc = htmlReadMemory([document bytes], [document length], "", "WINDOWS-1251  ", 0);
